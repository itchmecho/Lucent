...
enum L10n {
    ...
    enum Photos {
        ...
        static func deletePhotosTitle(_ count: Int) -> String {
            count == 1
                ...
                : String(localized: "photos.delete_photos_title \(count)", defaultValue: "Delete \(count) Photos?")
        }
        ...
        static func deletePhotosMessage(_ count: Int) -> String {
            count == 1
                ...
                : String(localized: "photos.delete_multiple_message \(count)", defaultValue: "These \(count) photos will be permanently deleted. This action cannot be undone.")
        }

        static func exportPhotosTitle(_ count: Int) -> String {
            count == 1
                ? String(localized: "photos.export_photo_title", defaultValue: "Export Photo?")
                : String(localized: "photos.export_photos_title \(count)", defaultValue: "Export \(count) Photos?")
        }
        ...
        static func exportPhotosMessage(_ count: Int) -> String {
            count == 1
                ...
                : String(localized: "photos.export_multiple_message \(count)", defaultValue: "These \(count) photos will be decrypted and saved to your photo library.")
        }
        ...
        static func photoCount(_ count: Int) -> String {
            count == 1
                ...
                : String(localized: "photos.photo_count \(count)", defaultValue: "\(count) photos")
        }
    }
    ...
    enum Import {
        ...
        static func importingProgress(_ current: Int, _ total: Int) -> String {
            String(localized: "import.importing_progress \(current) \(total)", defaultValue: "Importing \(current) of \(total)")
        }
    ...
    enum Auth {
        ...
        static func unlockWith(_ method: String) -> String {
            String(localized: "auth.unlock_with \(method)", defaultValue: "Unlock with \(method)")
        }
    ...
    enum Settings {
        ...
        static func useBiometricToUnlock(_ biometric: String) -> String {
            String(localized: "settings.use_biometric_to_unlock \(biometric)", defaultValue: "Use \(biometric) to unlock")
        }
